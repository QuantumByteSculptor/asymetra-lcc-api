"""
scripts/ml/train_v17_sec_fund.py
================================
V17 — Point-in-Time Fundamentals via SEC EDGAR XBRL + Regime Walk-Forward
...
log = logging.getLogger("train_v17")
...
def build_tech_features(
    ...
    for i, date in enumerate(df.index):
        ...
        fwd_stock.append(_fwd(p_i,   pos_p))
        fwd_spy_list.append(_fwd(spy_i, pos_spy))
...
def build_dataset_v17(
    ...
    log.info("Dataset v17: %d rows, %d tickers, %s → %s",
...
def run_walk_forward(df: pd.DataFrame, splits: List[Dict]) -> Dict:
    ...
    log.info("V17 training: %d features", len(feat_cols))
...
# ─────────────────────────────────────────────────────────────────────────────
def backtest_v17(oof_df: pd.DataFrame, top_pct: float = TOP_PCT) -> Dict:
    ...
    step   = 3  # non-overlapping 3m evaluation windows

    ret_reg_only: List[float] = []   # regime filter only
    ret_reg_mom:  List[float] = []   # regime + momentum filter
    ret_spy:      List[float] = []
    ...
    for i, month in enumerate(months):
        ...
        mom_ret = float(top_m["fwd_ret_3m"].mean()) * exposure + spy_ret * (1.0 - exposure)
    ...
    return {
        "v17_regime_only":         reg_m,
        "v17_regime_and_momentum": mom_m,
        "spy_benchmark":           spy_m,
        "per_regime":              per_regime,
...
def print_summary(results: Dict) -> None:
    ...
    reg  = bt["v17_regime_only"]
    mom  = bt["v17_regime_and_momentum"]
    spy  = bt["spy_benchmark"]

    baselines = {
        "V14-B": {"sharpe": 1.61,  "max_drawdown": -0.138, "cagr": None},
        "V16":   {"sharpe": 1.214, "max_drawdown": -0.145, "cagr": 0.222},
        "V15":   {"sharpe": 1.167, "max_drawdown": -0.147, "cagr": 0.198},
    }

    print("\n" + "=" * 78)
    print("  V17 — SEC EDGAR POINT-IN-TIME FUNDAMENTALS + REGIME WALK-FORWARD")
    print("=" * 78)
    print(f"{'Metric':<22} {'V17 reg':>10} {'V17 mom':>10} {'V16':>8} {'V14-B':>8} {'SPY':>8}")
    print("-" * 70)
    ...
    for label, key, pct in [
        ...
        print(f"{label:<22} {_fmt(reg,key,pct):>10} {_fmt(mom,key,pct):>10} "
              f"{_fmt(baselines['V16'],key,pct):>8} {_fmt(baselines['V14-B'],key,pct):>8} "
...
# ─────────────────────────────────────────────────────────────────────────────
def main() -> None:
    ap = argparse.ArgumentParser(description="V17 SEC EDGAR point-in-time fundamentals training")
    ap.add_argument("--start",             default="2017-01-01")
    ap.add_argument("--end",               default="2024-12-31")
    ap.add_argument("--out",               default="data/metrics/v17_results.json")
    ap.add_argument("--model_out",         default="models/stock_picker_v17.joblib")
    ...
    reg_sharpe = bt["v17_regime_only"].get("sharpe", 0) or 0
    mom_sharpe = bt["v17_regime_and_momentum"].get("sharpe", 0) or 0
    best_variant = "v17_regime_only" if reg_sharpe >= mom_sharpe else "v17_regime_and_momentum"
    best_bt      = bt["v17_regime_only"] if best_variant == "v17_regime_only" else bt["v17_regime_and_momentum"]
    ...
    results = {
        ...
        "config": {
            ...
            "ttm_method": "sum 4 standalone quarterly filings (dur 75-110d); fallback: annual FY",
        },
        ...
        "comparison": {
            "v17_best_sharpe":       best_bt.get("sharpe"),
            "v17_best_max_drawdown": best_bt.get("max_drawdown"),
            "v17_best_cagr":         best_bt.get("cagr"),
            "v17_best_sortino":      best_bt.get("sortino"),
            "v17_best_calmar":       best_bt.get("calmar"),
            "v16_sharpe":            1.214,
            "v16_max_drawdown":      -0.145,
            "v15_sharpe":            1.167,
            "v15_max_drawdown":      -0.147,
            "v14b_sharpe":           1.61,
            "v14b_max_drawdown":     -0.138,
            "delta_sharpe_vs_v16":   round((best_bt.get("sharpe") or 0) - 1.214, 3),
    ...
    if wf["models"]:
        ...
        bundle = {
            "model_version":      "v17",
            "feature_version":    "v17_edgar_regime",
