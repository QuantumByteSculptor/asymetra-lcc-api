"""
scripts/ml/train_v20b_bear_alpha.py
=====================================
V20b — V20a + two changes that close the gap vs V14-B (1.610 monthly):
  ...
  1. REGIME_EXPOSURE[bear] = 0.5  (was 0.0)
     ...
     V20b reduces exposure to 50% in bear markets but still picks top stocks.
...
Base: V20a (monthly backtest, sqrt(12), EDGAR cache edgar_v17).
Target: beat V14-B Sharpe 1.610 with MaxDD < -18%.

Usage:
  python scripts/ml/train_v20b_bear_alpha.py \\
      --start 2017-01-01 --end 2024-12-31 \\
      --out   data/metrics/v20b_results.json \\
      --model_out models/stock_picker_v20b.joblib \\
...
log = logging.getLogger("train_v20b")
...
REGIME_EXPOSURE = {REGIME_BULL: 1.0, REGIME_SIDEWAYS: 0.5, REGIME_BEAR: 0.5}  # V20b: pick in bear too
...
TECH_FEATURES = [
    ...
    "ret_12m_vs_spy",  # V20b: relative 12m alpha momentum (as in V14-B)
    "spy_1m", "spy_12m",
...
V20B_FEATURES = TECH_FEATURES + REGIME_FEATURES + FUND_FEATURES + STATIC_FEATURES
...
def build_dataset_v17(
    ...
    log.info("Dataset v20b: %d rows, %d tickers, %s → %s",
...
def run_walk_forward(df: pd.DataFrame, splits: List[Dict]) -> Dict:
    feat_cols = [c for c in V20B_FEATURES if c in df.columns]
    log.info("V20b training: %d features", len(feat_cols))
...
# ─────────────────────────────────────────────────────────────────────────────
def backtest_v20b(oof_df: pd.DataFrame, top_pct: float = TOP_PCT) -> Dict:
    """Monthly-step backtest (step=1, ppy=12) using 1m forward returns.
    V20b: bear exposure = 0.5 (stock picking in bear markets too)."""
    ...
    return {
        "v20b_regime_only":         reg_m,
        "v20b_regime_and_momentum": mom_m,
        "spy_benchmark":           spy_m,
        "per_regime":              per_regime,
...
def print_summary(results: Dict) -> None:
    ...
    reg  = bt["v20b_regime_only"]
    mom  = bt["v20b_regime_and_momentum"]
    ...
    print("\n" + "=" * 78)
    print("  V20b — MONTHLY BACKTEST (step=1, sqrt(12)) — comparable to V14-B monthly")
    print("=" * 78)
    print(f"{'Metric':<22} {'V20b reg':>10} {'V20b mom':>10} {'V17 (Q)':>8} {'V14-B':>8} {'SPY':>8}")
    print("-" * 70)
...
# ─────────────────────────────────────────────────────────────────────────────
def main() -> None:
    ap = argparse.ArgumentParser(description="V20b monthly backtest (bear=50% + ret_12m_vs_spy)")
    ap.add_argument("--start",             default="2017-01-01")
    ap.add_argument("--end",               default="2024-12-31")
    ap.add_argument("--out",               default="data/metrics/v20b_results.json")
    ap.add_argument("--model_out",         default="models/stock_picker_v20b.joblib")
    ...
    reg_sharpe = bt["v20b_regime_only"].get("sharpe", 0) or 0
    mom_sharpe = bt["v20b_regime_and_momentum"].get("sharpe", 0) or 0
    best_variant = "v20b_regime_only" if reg_sharpe >= mom_sharpe else "v20b_regime_and_momentum"
    best_bt      = bt["v20b_regime_only"] if best_variant == "v20b_regime_only" else bt["v20b_regime_and_momentum"]
    ...
    results = {
        ...
        "config": {
            ...
            "n_test_months": args.n_test_months,
            "min_train_months": args.min_train_months,
            "features": V20B_FEATURES, "n_features": len(V20B_FEATURES),
            "backtest_step": 1,
            "backtest_ppy":  12,
        ...
        "comparison": {
            "v20b_best_sharpe":      best_bt.get("sharpe"),
            "v20b_best_max_drawdown":best_bt.get("max_drawdown"),
            "v20b_best_cagr":        best_bt.get("cagr"),
            "v20b_best_sortino":     best_bt.get("sortino"),
            "v20b_best_calmar":      best_bt.get("calmar"),
    ...
    if wf["models"]:
        ...
        bundle = {
            "model_version":      "v20b",
            "feature_version":    "v20b_bear_alpha_monthly",
