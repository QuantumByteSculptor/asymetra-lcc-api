"""
scripts/ml/train_v20c_universe.py
=====================================
V20c — V20b + expanded universe (150 tickers) that close the gap vs V14-B (1.610 monthly):

  1. REGIME_EXPOSURE[bear] = 0.5  (was 0.0)
     V14-B picks stocks in ALL regimes (incl. bear); V17/V20a goes to cash in
     bear markets and misses bear-market alpha (e.g. energy/defense in 2022).
     V20c retains bear exposure to 50% in bear markets but still picks top stocks.

  2. Feature `ret_12m_vs_spy` = ret_12m - spy_12m  (relative alpha momentum)
     V14-B has this; V17 doesn't. Cross-sectional relative momentum is a stronger
     signal than absolute momentum for identifying outperformers.

Base: V20b (monthly backtest, sqrt(12), EDGAR cache edgar_v20c).
Target: beat V14-B Sharpe 1.610 with MaxDD < -18%.

Usage:
  python scripts/ml/train_v20c_universe.py \\
      --start 2017-01-01 --end 2024-12-31 \\
      --out   data/metrics/v20c_results.json \\
      --model_out models/stock_picker_v20c.joblib \\
      --cache data/cache/edgar_v20c
...
log = logging.getLogger("train_v20c")

SEED = 42
np.random.seed(SEED)

# ─────────────────────────────────────────────────────────────────────────────
# Universe — 150 tickers (V20c: 3× expanded vs V20b's 50)
# Sectors: 0=Tech, 1=Finance, 2=Healthcare, 3=Consumer, 4=Energy,
#          5=Industrial, 6=Utilities, 7=RealEstate, 8=Materials
# ─────────────────────────────────────────────────────────────────────────────
UNIVERSE = [
    # Tech (25)
    "AAPL", "MSFT", "GOOGL", "META", "AMZN", "NVDA", "AMD", "INTC", "CSCO", "ORCL",
    "QCOM", "TXN",  "MU",    "AVGO", "AMAT", "KLAC", "ADI", "INTU", "IBM",  "ACN",
    "CRM",  "ADBE", "NOW",   "PANW", "HPQ",
    # Finance (25)
    "JPM",  "BAC",  "WFC",   "GS",   "MS",   "BRK-B","C",   "AXP",  "BLK",  "USB",
    "PNC",  "TFC",  "SCHW",  "BK",   "NTRS", "AIG",  "ALL", "PRU",  "MET",  "TRV",
    "AFL",  "CB",   "FITB",  "KEY",  "RF",
    # Healthcare (25)
    "JNJ",  "UNH",  "PFE",   "ABBV", "MRK",  "LLY",  "TMO", "ABT",  "MDT",  "CVS",
    "ISRG", "SYK",  "BSX",   "DHR",  "BDX",  "BIIB", "REGN","VRTX", "GILD", "AMGN",
    "HUM",  "CI",   "MCK",   "MOH",  "CNC",
    # Consumer (25)
    "WMT",  "PG",   "KO",    "PEP",  "MCD",  "COST", "NKE", "HD",   "TGT",  "LOW",
    "SBUX", "TJX",  "ROST",  "YUM",  "DRI",  "CMG",  "TSCO","DLTR", "DG",   "CLX",
    "MKC",  "MDLZ", "KHC",   "GIS",  "STZ",
    # Energy (15)
    "XOM",  "CVX",  "COP",   "SLB",  "PSX",  "MPC",  "VLO", "OXY",  "HAL",  "DVN",
    "EOG",  "HES",  "PXD",   "BKR",  "FCX",
    # Industrial (15)
    "CAT",  "HON",  "MMM",   "GE",   "BA",   "UPS",  "UNP", "CSX",  "NSC",  "FDX",
    "RTX",  "LMT",  "NOC",   "GD",   "EMR",
    # Utilities (5)
    "NEE",  "DUK",  "SO",    "D",    "AEP",
    # Real Estate (5)
    ...
    # Materials (10)
    "LIN",  "APD",  "SHW",   "ECL",  "NEM",  "NUE",  "VMC", "MLM",  "PPG",  "IP",
]
...
SECTOR_MAP = {
    ...
    "QCOM": 0, "TXN":  0, "MU":    0, "AVGO": 0, "AMAT": 0,
    "KLAC": 0, "ADI":  0, "INTU":  0, "IBM":  0, "ACN":  0,
    "CRM":  0, "ADBE": 0, "NOW":   0, "PANW": 0, "HPQ":  0,
    # Finance = 1
    "JPM":  1, "BAC":  1, "WFC":   1, "GS":   1, "MS":   1,
    "BRK-B":1, "C":    1, "AXP":   1, "BLK":  1, "USB":  1,
    "PNC":  1, "TFC":  1, "SCHW":  1, "BK":   1, "NTRS": 1,
    "AIG":  1, "ALL":  1, "PRU":   1, "MET":  1, "TRV":  1,
    "AFL":  1, "CB":   1, "FITB":  1, "KEY":  1, "RF":   1,
    # Healthcare = 2
    "JNJ":  2, "UNH":  2, "PFE":   2, "ABBV": 2, "MRK":  2,
    "LLY":  2, "TMO":  2, "ABT":   2, "MDT":  2, "CVS":  2,
    "ISRG": 2, "SYK":  2, "BSX":   2, "DHR":  2, "BDX":  2,
    "BIIB": 2, "REGN": 2, "VRTX":  2, "GILD": 2, "AMGN": 2,
    "HUM":  2, "CI":   2, "MCK":   2, "MOH":  2, "CNC":  2,
    # Consumer = 3
    "WMT":  3, "PG":   3, "KO":    3, "PEP":  3, "MCD":  3,
    "COST": 3, "NKE":  3, "HD":    3, "TGT":  3, "LOW":  3,
    "SBUX": 3, "TJX":  3, "ROST":  3, "YUM":  3, "DRI":  3,
    "CMG":  3, "TSCO": 3, "DLTR":  3, "DG":   3, "CLX":  3,
    "MKC":  3, "MDLZ": 3, "KHC":   3, "GIS":  3, "STZ":  3,
    # Energy = 4
    "XOM":  4, "CVX":  4, "COP":   4, "SLB":  4, "PSX":  4,
    "MPC":  4, "VLO":  4, "OXY":   4, "HAL":  4, "DVN":  4,
    "EOG":  4, "HES":  4, "PXD":   4, "BKR":  4, "FCX":  4,
    # Industrial = 5
    "CAT":  5, "HON":  5, "MMM":   5, "GE":   5, "BA":   5,
    "UPS":  5, "UNP":  5, "CSX":   5, "NSC":  5, "FDX":  5,
    "RTX":  5, "LMT":  5, "NOC":   5, "GD":   5, "EMR":  5,
    # Utilities = 6
    "NEE":  6, "DUK":  6, "SO":    6, "D":    6, "AEP":  6,
    # Real Estate = 7
    "PLD":  7, "AMT":  7, "CCI":   7, "EQIX": 7, "SPG":  7,
    # Materials = 8
    "LIN":  8, "APD":  8, "SHW":   8, "ECL":  8, "NEM":  8,
    "NUE":  8, "VMC":  8, "MLM":   8, "PPG":  8, "IP":   8,
}
...
REGIME_EXPOSURE = {REGIME_BULL: 1.0, REGIME_SIDEWAYS: 0.5, REGIME_BEAR: 0.5}  # inherited from V20b
...
TECH_FEATURES = [
    ...
    "ret_12m_vs_spy",  # inherited from V20b (as in V14-B)
    "spy_1m", "spy_12m",
...
V20C_FEATURES = TECH_FEATURES + REGIME_FEATURES + FUND_FEATURES + STATIC_FEATURES
...
def build_tech_features(
    ...
    for i, date in enumerate(p.index):
        ...
        ret_12m_vs_spy = (ret_12m - spy_12m) if all(pd.notna(x) for x in [ret_12m, spy_12m]) else float("nan")
        ...
        rows.append({
            ...
            "mom_12_1": mom_12_1, "ret_vs_spy_3m": ret_vs_spy3, "ret_12m_vs_spy": ret_12m_vs_spy,
...
def build_dataset_v17(
    ...
    log.info("Dataset v20c: %d rows, %d tickers, %s → %s",
...
def run_walk_forward(df: pd.DataFrame, splits: List[Dict]) -> Dict:
    feat_cols = [c for c in V20C_FEATURES if c in df.columns]
    log.info("V20c training: %d features", len(feat_cols))
...
# ─────────────────────────────────────────────────────────────────────────────
def backtest_v20c(oof_df: pd.DataFrame, top_pct: float = TOP_PCT) -> Dict:
    """Monthly-step backtest (step=1, ppy=12) using 1m forward returns.
    V20c: 150-ticker universe, bear exposure = 0.5."""
    ...
    return {
        "v20c_regime_only":         reg_m,
        "v20c_regime_and_momentum": mom_m,
        "spy_benchmark":           spy_m,
        "per_regime":              per_regime,
...
def print_summary(results: Dict) -> None:
    ...
    reg  = bt["v20c_regime_only"]
    mom  = bt["v20c_regime_and_momentum"]
    ...
    print("\n" + "=" * 78)
    print("  V20c — MONTHLY BACKTEST (step=1, sqrt(12)) — comparable to V14-B monthly")
    print("=" * 78)
    print(f"{'Metric':<22} {'V20c reg':>10} {'V20c mom':>10} {'V17 (Q)':>8} {'V14-B':>8} {'SPY':>8}")
    print("-" * 70)
...
# ─────────────────────────────────────────────────────────────────────────────
def main() -> None:
    ap = argparse.ArgumentParser(description="V20c monthly backtest (bear=50% + ret_12m_vs_spy)")
    ap.add_argument("--start",             default="2017-01-01")
    ap.add_argument("--end",               default="2024-12-31")
    ap.add_argument("--out",               default="data/metrics/v20c_results.json")
    ap.add_argument("--model_out",         default="models/stock_picker_v20c.joblib")
    ap.add_argument("--cache",             default="data/cache/edgar_v20c")
    ...
    reg_sharpe = bt["v20c_regime_only"].get("sharpe", 0) or 0
    mom_sharpe = bt["v20c_regime_and_momentum"].get("sharpe", 0) or 0
    best_variant = "v20c_regime_only" if reg_sharpe >= mom_sharpe else "v20c_regime_and_momentum"
    best_bt      = bt["v20c_regime_only"] if best_variant == "v20c_regime_only" else bt["v20c_regime_and_momentum"]
    ...
    results = {
        ...
        "config": {
            ...
            "n_test_months": args.n_test_months,
            "min_train_months": args.min_train_months,
            "features": V20C_FEATURES, "n_features": len(V20C_FEATURES),
            "backtest_step": 1,
            "backtest_ppy":  12,
        ...
        "comparison": {
            "v20c_best_sharpe":      best_bt.get("sharpe"),
            "v20c_best_max_drawdown":best_bt.get("max_drawdown"),
            "v20c_best_cagr":        best_bt.get("cagr"),
            "v20c_best_sortino":     best_bt.get("sortino"),
            "v20c_best_calmar":      best_bt.get("calmar"),
    ...
    if wf["models"]:
        ...
        bundle = {
            "model_version":      "v20c",
            "feature_version":    "v20c_universe_monthly",
